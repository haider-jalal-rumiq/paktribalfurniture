...
try {
  for (let i = 0; i < 120; i++) {
    try { const response = await fetch(BASE + "/cms/login"); if (response.ok) break; } catch {}
  ...
  await page.waitForURL(BASE + "/cms");
  ...
  await go(page, `/cms/orders/${fixture.order.id}`);
  ...
  await go(page, "/cms/expenses/labour/new?month=2026-09");
  ...
  await go(page, "/cms/invoices/new");
  ...
  await go(page, `/cms/invoices/${created.id}/edit`);
  await page.getByLabel("Unit amount (Rs)", { exact: true }).fill("4000");
  await page.getByRole("button", { name: "Save invoice", exact: true }).click();
  await page.waitForURL(`${BASE}/cms/invoices/${created.id}`);
  ...
  await go(page, `/cms/invoices?client=${fixture.client.id}&from=2026-09-20&to=2026-09-26`);
  ...
  await go(page, "/cms/invoices?from=2026-09-27&to=2026-09-20");
  ...
  for (const [name, path] of [["invoice", `/cms/invoices/${fixture.db.invoices[0].id}`], ["order", `/cms/orders/${fixture.order.id}/print`], ["labour", "/cms/expenses/labour/print?month=2026-09"]]) {
  ...
  await go(page, `/cms/invoices/print?client=${fixture.client.id}&from=2026-09-20&to=2026-09-26`);
  ...
  await go(page, `/cms/invoices/${fixture.db.invoices[0].id}`);
  await page.emulateMedia({ media: "print" });
  ...
  for (const [name, path] of [["dashboard", "/cms"], ["orders", "/cms/orders"], ["invoices", "/cms/invoices"], ["labour", "/cms/expenses/labour?month=2026-09"], ["invoice", `/cms/invoices/${fixture.db.invoices[0].id}`]]) {
    await go(page, path);
