...
export function FeaturedProducts({ products }: { products: Product[] }) {
  ...
  return (
    <section className="py-2 sm:py-8">
      <Container>
        <MaskReveal className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="mt-12 grid gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
          {products.slice(0, 6).map((product, index) => (
            <TileReveal key={product.id} index={index}><ProductCard product={product} /></TileReveal>
          ))}
        </div>
      </Container>
    </section>
