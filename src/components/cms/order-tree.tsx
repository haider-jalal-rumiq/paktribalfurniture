...
export function OrderItems({ items }: { items: OrderItem[] }) {
  return items.length ? <ol className="divide-y divide-hairline">
    {items.map((item, index) => <li key={item.id} className="flex flex-wrap items-start gap-3 py-3">
      ...
      {item.amount ? <span className="shrink-0 text-sm font-semibold tabular-nums text-ink-soft">{formatPkr(orderTotal([item]))}</span> : null}
...
export function OrderTree({ orders, clients, expandClients = false }: { orders: OrderWithClient[]; clients: Pick<Client, "id" | "name" | "type">[]; expandClients?: boolean }) {
  ...
  return <div className="space-y-4">
    {clients.map((client) => {
      ...
      const clientValue = clientOrders.reduce((sum, order) => sum + orderTotal(order.items), 0n);
        ...
        <div className="border-t border-hairline px-3 pb-3 sm:px-5">
          {clientOrders.map((order) => <details key={order.id} className={cn("order-branch mt-3 rounded-[var(--radius-ui)] border border-hairline", isUrgentOrder(order) && "border-l-4 border-accent/40 border-l-accent bg-accent/[0.05]")}>
            <summary className="flex cursor-pointer list-none flex-wrap items-center gap-x-3 gap-y-2 px-3 py-4">
              ...
              {orderTotal(order.items) > 0n && <span className="shrink-0 text-sm font-bold tabular-nums text-accent">{formatPkr(orderTotal(order.items))}</span>}
