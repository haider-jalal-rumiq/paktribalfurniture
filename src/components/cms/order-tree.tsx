    ...
    {clients.map((client) => {
      ...
      return <details key={client.id} open={expandClients} className="order-client overflow-hidden rounded-[var(--radius-card)] border border-hairline bg-surface">
        <summary className="flex min-h-20 cursor-pointer list-none items-center gap-4 px-4 py-4 sm:px-5">
          ...
          <span className="min-w-0 flex-1"><span className="block break-words font-semibold text-ink">{client.name}</span><span className="mt-1 block text-xs text-muted">{clientTypeShort(client.type)} · {clientOrders.length} orders · {open} open · {complete} completed</span></span>
        ...
        <div className="border-t border-hairline px-3 pb-3 sm:px-5">
          {clientOrders.map((order) => <details key={order.id} className="order-branch mt-3 rounded-[var(--radius-ui)] border border-hairline">
            <summary className="flex cursor-pointer list-none items-center gap-3 px-3 py-4">
              <span className="min-w-0 flex-1"><span className="block break-words text-sm font-bold text-ink">#{order.order_no} · {order.title}</span><span className="mt-1 block text-xs text-muted">{order.items.length} items{order.site_label ? ` · ${order.site_label}` : ""}</span></span>
              <Badge tone={STATUS_TONE[order.status]}>{orderStatusLabel(order.status)}</Badge>
              <ChevronRight className="tree-chevron h-4 w-4 shrink-0 text-accent" aria-hidden="true" />
            </summary>
            <div className="border-t border-hairline px-4"><OrderItems items={order.items} /><Link href={`/cms/orders/${order.id}`} className="my-2 inline-flex min-h-11 items-center text-sm font-semibold text-accent">Open / edit order <ChevronRight className="ml-1 h-4 w-4" aria-hidden="true" /></Link></div>
          </details>)}
          {!clientOrders.length && <p className="p-4 text-sm text-muted">No orders for this client yet.</p>}
          <Link className="mt-3 inline-flex min-h-11 items-center text-sm font-semibold text-accent" href={`/cms/orders/new?client=${client.id}`}>Add an order for {client.name}</Link>
        </div>
      </details>;
    })}
  </div>;
