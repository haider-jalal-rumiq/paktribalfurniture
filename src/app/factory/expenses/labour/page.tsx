...
export default async function LabourPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  return <CmsPage title="Labour sheet" eyebrow="Expenses" actions={<><ButtonLink href={`/factory/expenses/labour/new?month=${month}`} size="sm">Add labour entry</ButtonLink>{entries.length > 0 && <ButtonLink href={`/factory/expenses/labour/print?month=${month}`} size="sm" variant="outline">Print / PDF</ButtonLink>}</>}>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{rows.map(({ entry, totals }) => <Link key={entry.id} href={`/factory/expenses/labour/${entry.id}`} className="rounded-[var(--radius-card)] border border-hairline p-5 transition-colors hover:border-accent"><div className="flex flex-wrap justify-between gap-3"><h2 className="break-words font-display text-xl">{entry.name}</h2><span className="text-xs text-muted">{entry.leaves} leave days{entry.ot_hours > 0 ? ` · ${entry.ot_hours} h OT` : ""}</span></div><dl className="mt-5 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">{[{ label: "Salary", amount: BigInt(entry.salary) }, { label: "Overtime", amount: totals.overtime }, { label: "Deductions", amount: totals.leaveDeduction + BigInt(entry.deduction) }, { label: "Total payable", amount: totals.total }, { label: "Paid", amount: totals.paid }, { label: "Balance", amount: totals.balance }].map((field) => <div key={field.label}><dt className="text-xs text-muted">{field.label}</dt><dd className="mt-1 break-all font-semibold tabular-nums">{formatPkr(field.amount)}</dd></div>)}</dl><p className="mt-4 text-xs text-muted">Payment date: {showDate(entry.paid_on)}</p></Link>)}</div>
