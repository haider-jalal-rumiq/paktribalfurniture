...
export default async function ClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <CmsPage backHref="/factory/clients" eyebrow={clientTypeLabel(client.type)} title={client.name} actions={<><ButtonLink href={`/factory/orders/new?client=${id}`} size="sm">New order</ButtonLink><ButtonLink href={`/factory/invoices?client=${id}`} variant="outline" size="sm">Client invoices</ButtonLink></>}>
