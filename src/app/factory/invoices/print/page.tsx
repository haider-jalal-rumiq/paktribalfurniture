...
export default async function PrintInvoicesPage({ searchParams }: { searchParams: Promise<{ client?: string; from?: string; to?: string; status?: string }> }) {
  ...
  return <CmsPage title="Invoice report" backHref={`/factory/invoices?${query}`} actions={invoices.length ? <PrintButton /> : undefined}>
    ...
    </Document>{invoices.map((invoice) => <InvoiceDocument key={invoice.id} invoice={invoice} reference={invoiceNumber(invoice.invoice_no)} />)}</>}
