...
import { labourAmounts, showDate, sumRupees } from "@/lib/accounting-core";
...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  const labourPaid = labour.reduce((sum, row) => sum + labourAmounts(row).paid, 0n);
  ...
  return <CmsPage title="Expenses" eyebrow="Workshop">
    ...
    <form action="/cms/expenses" className="mb-6 flex items-end gap-3"><Field label="Expense month" htmlFor="expenseMonth"><Input id="expenseMonth" name="month" type="month" defaultValue={month} required /></Field><Button type="submit" variant="outline">Show</Button></form>
    ...
    <section className="mt-9"><SectionHeading>{monthLabel(month)} entries</SectionHeading>
      ...
      {labour.length > 0 && <p className="mt-5 text-sm text-muted">{labour.length} labour entries paid this month. <Link href="/cms/expenses/labour" className="font-semibold text-accent">Open labour sheet</Link></p>}
    </section>
  </CmsPage>;
}
