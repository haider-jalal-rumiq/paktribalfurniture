...
export default async function LabourEntryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!entry) notFound();
  const back = `/cms/expenses/labour?month=${entry.period.slice(0, 7)}`;
  return <CmsPage title={entry.name} eyebrow="Labour entry" backHref={back} actions={<ButtonLink size="sm" variant="outline" href={`/cms/expenses/labour/print?month=${entry.period.slice(0, 7)}&entry=${id}`}>Print / PDF</ButtonLink>}>
