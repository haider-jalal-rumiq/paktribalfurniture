...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!invoice) notFound();
  return <CmsPage title={invoiceNumber(invoice.invoice_no)} eyebrow={invoice.client_name} backHref="/cms/invoices" actions={<><PrintButton />{invoice.status === "issued" && <ButtonLink size="sm" href={`/cms/invoices/${id}/edit`}>Edit invoice</ButtonLink>}</>}>
