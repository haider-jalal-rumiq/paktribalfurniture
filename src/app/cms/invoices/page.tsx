...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<{ client?: string; from?: string; to?: string; status?: string }> }) {
  ...
  return <CmsPage title="Invoices" eyebrow="Sales" actions={<><ButtonLink href={`/cms/invoices/new${filters.client ? `?client=${filters.client}` : ""}`} size="sm"><Plus className="h-4 w-4" aria-hidden="true" />New invoice</ButtonLink>{invoices.length > 0 && <ButtonLink href={`/cms/invoices/print?${query}`} size="sm" variant="outline">Print / PDF results</ButtonLink>}</>}>
    <form action="/cms/invoices" className="mb-6 grid items-end gap-3 rounded-[var(--radius-card)] border border-hairline p-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <div className="flex items-center gap-3"><Button type="submit" variant="outline">Filter</Button><Link className="text-sm text-muted" href="/cms/invoices">Reset</Link></div>
    </form>
    ...
    {grouped.map((client) => <section key={client.id} className="mb-6 overflow-hidden rounded-[var(--radius-card)] border border-hairline"><h2 className="bg-canvas-deep px-4 py-4 font-display text-xl">{client.name}</h2><ul className="divide-y divide-hairline">{invoices.filter((row) => row.client_id === client.id).map((row) => <li key={row.id}><Link className="flex flex-wrap items-center justify-between gap-3 p-4 hover:bg-wash/50" href={`/cms/invoices/${row.id}`}><div><p className="text-sm font-bold">{invoiceNumber(row.invoice_no)}</p><p className="mt-1 text-xs text-muted">{showDate(row.issued_on)} · {row.items.length} items</p></div><span className="break-all text-sm font-semibold tabular-nums">{formatPkr(row.total_amount)}</span></Link></li>)}</ul></section>)}
