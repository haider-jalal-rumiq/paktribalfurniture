...
export default async function DashboardPage() {
  ...
  return <CmsPage eyebrow="Pak Tribal Furniture" title="Dashboard" actions={<>
    <ButtonLink href="/cms/balances" size="sm"><Plus className="h-4 w-4" aria-hidden="true" />Add balance</ButtonLink>
    <ButtonLink href="/cms/invoices/new" size="sm" variant="outline">New invoice</ButtonLink>
  </>}>
    ...
    <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm"><Link href="/cms/balances" className="min-h-11 py-3 font-semibold text-accent">Balance history{totals ? ` · ${formatPkr(totals.added)} added` : ""}</Link><Link href="/cms/expenses" className="min-h-11 py-3 font-semibold text-accent">Record an expense</Link><Link href="/cms/expenses/labour" className="min-h-11 py-3 font-semibold text-accent">Labour sheet</Link></div>
    <section className="mt-9">
      <SectionHeading action={<Link href="/cms/orders" className="text-sm font-semibold text-accent">All orders</Link>}>Due soon</SectionHeading>
      {dueSoon.length ? <RecordList empty="No upcoming deliveries." rows={dueSoon.map((order) => {
        const days = daysUntil(order.expected_date!);
        return { id: order.id, href: `/cms/orders/${order.id}`, title: order.title, subtitle: `#${order.order_no} · ${order.clients?.name ?? "Client"}`, meta: <Badge tone={STATUS_TONE[order.status]}>{orderStatusLabel(order.status)}</Badge>, metaSub: <span className={days < 0 ? "font-semibold text-accent" : "text-muted"}>{days < 0 ? `${Math.abs(days)} days overdue` : days === 0 ? "Due today" : `Due in ${days} days`}</span> };
