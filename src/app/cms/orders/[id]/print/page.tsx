...
export default async function PrintOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!order) notFound();
  return <CmsPage title={`Order #${order.order_no}`} backHref={`/cms/orders/${id}`} actions={<PrintButton />}><p className="print-controls mb-5 text-sm text-muted">Choose Save as PDF in the print window to download and share this order.</p><OrderDocument order={order} /></CmsPage>;
}
